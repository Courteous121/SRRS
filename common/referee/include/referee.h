...
#include  "../SerialPort/include/DataStruct.h"
#include  "../SerialPort/include/SerialPort.h"
#ifdef __cplusplus
/* Private define ------------------------------------------------------------*/

#define DRAWING_PACK	15
#define SEND_PACK_NUM 	23
...
class referee_Classdef
{
	public:
		...
		/* 空中机器人pitch轴标尺 */
		void Draw_Pitch_Ruler(uint8_t _layer, uint16_t center_x, uint16_t center_y, uint16_t total_length, uint16_t long_scale_length, uint16_t short_scale_length, colorType_e ruler_color, colorType_e current_color, colorType_e target_color);
		/* 空中机器人pitch轴浮标 */
		void Pitch_Current_Point(float pitch_angle, uint8_t _layer, uint16_t center_x, uint16_t center_y, uint16_t total_length, colorType_e tag_color);
		/* 空中机器人pitch轴目标浮标 */
		void Pitch_Target_Point(float pitch_target, uint8_t _layer, uint16_t center_x, uint16_t center_y, uint16_t total_length, colorType_e target_color);
		
		/* 工程机器人抬升高度标尺 */
		void Engineer_Height_Ruler(uint8_t _layer, uint16_t center_x, uint16_t center_y, uint16_t total_length, uint16_t long_scale_length, uint16_t short_scale_length, uint8_t ruler_tag, colorType_e ruler_color, colorType_e current_color);
		/* 工程机器人当前抬升高度浮标 */
		void Engineer_Current_Height(float height, uint8_t _layer, uint16_t center_x, uint16_t center_y, uint16_t total_length, uint8_t ruler_tag, colorType_e tag_color);
		/* 工程机器人当前抬升高度浮标（两个标尺，同个数据包发送） */
		void Engineer_Current_HeightMul(float *height, uint8_t _layer, uint16_t *center_x, uint16_t *center_y, uint16_t *total_length, colorType_e tag_color);
		/* 工程机器人目标抬升高度浮标 */
		...
		/* 矿石UI */
		void Mine(uint8_t _layer, uint16_t center_x, uint16_t center_y, uint8_t size, uint8_t mine_tag, colorType_e _color);
		/* 工程机器人当前存储矿石数UI框架 */
		void Mine_Remain_Frame(uint8_t _layer, uint16_t start_x, uint16_t start_y);
		/* 更新工程机器人存储矿石数 */
		void Mine_Remain_Update(uint8_t mine_num, uint8_t _layer, uint16_t start_x, uint16_t start_y);
		/* 工程机器人自动/手动组状态 */
		void Engineer_Miner_Mode(uint8_t mode, uint8_t _layer, uint16_t start_x, uint16_t start_y, colorType_e _color);
		/* 工程机器人当前取矿状态 */
		void Engineer_Miner_Status(uint8_t status, uint8_t _layer, uint16_t start_x, uint16_t start_y);

		/* 哨兵机器人移动轨道绘制 */
		void Sentry_Position_Ruler(uint8_t _layer, uint16_t center_x, uint16_t center_y, uint16_t total_length, colorType_e ruler_color, colorType_e current_color);
		/* 哨兵机器人当前位置浮标 */
		void Sentry_Current_Pos(float pos_percent, uint8_t _layer, uint16_t center_x, uint16_t center_y, uint16_t total_length, colorType_e tag_color);
		/* 哨兵机器人巡逻区域绘制 */
		void Sentry_Patrol_Area(uint8_t _layer, uint16_t center_x, uint16_t center_y, uint16_t size, uint8_t *keys, colorType_e patrol_color);
		/* 哨兵机器人索敌状态指示 */
		void Sentry_Patrol_Status(uint8_t _layer, uint16_t center_x, uint16_t center_y, uint16_t size, uint8_t tag, colorType_e _color);
		/* 哨兵机器人发弹状态框架 */
		void Sentry_Bullet_Frame(uint8_t _layer, uint16_t start_x, uint16_t start_y, uint16_t size, colorType_e _color);
		/* 哨兵机器人发弹状态 */
		void Sentry_Bullet_Status(uint8_t _layer, uint16_t start_x, uint16_t start_y, uint16_t size, uint8_t tag, colorType_e _color);
		
		/* 雷达站策略集显示 */
		// void Radar_Strategy(uint8_t _layer, uint16_t start_x, uint16_t start_y, uint8_t tag, );
		...
		void send_toReferee(uint16_t _com_id,uint8_t* _data, uint16_t length, receive_Type_e _receive_type);
